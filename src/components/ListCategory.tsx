...
const ListCategory = ({categories, selectedCategory, setSelectedCategory} : Props) => {
  return (
    <div className="w-full bg-white p-5">
        ...
        <RadioGroup value={selectedCategory || ""} className="mt-2">
          {categories.map((item) => (
            <div 
              key={item?._id}
              onClick={() => setSelectedCategory(item?.slug?.current as string)}
            ...
            >
              ...
              <Label
                htmlFor={item._id}
                className={`${selectedCategory === item?.slug?.current ? "font-semibold text-shop_dark_green" : "font-normal"}`}
              >
                {item?.title}
              </Label>
            </div>
